...
📋 Deploy the default web app from `ingress/specs/default`:
...
📋 Now deploy the ingress rule in `ingress/specs/default/ingress` and list all rules:
...
📋 Deploy the app in `ingress/specs/whoami` and check the Ingress rules.
...
📋 Deploy the app in `ingress/specs/pi`, check the status and add `pi.local` to your hosts file.
...
```
kubectl apply -f ingress/specs/pi/update
```
